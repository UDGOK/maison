...
* everything else (``list``, ``summary``, ``respond``) needs a Maison role. The doctype has no
...
Office via Notification Log (+ e-mail when configured) and ``maison_feedback_alert`` realtime.
"""
...
from maison_pos.scoping import ALL_MAISON_ROLES, assert_boutique_access, assert_roles, get_user_boutique, is_unrestricted
...
def status(token: str) -> dict[str, Any]:
	...
	return {"enabled": True, "valid": True, "submitted": bool(frappe.db.exists("Maison Feedback", {"sales_invoice": doc.name}))}
...
def submit(token: str, rating: Any, comment: Optional[str] = None) -> dict[str, Any]:
	...
	fb = frappe.get_doc(
		{
			...
			"associate": doc.get("maison_associate") if frappe.db.exists("Maison Associate", doc.get("maison_associate") or "") else None,
	...
	frappe.publish_realtime("maison_feedback", {"boutique": fb.boutique, "rating": rating, "name": fb.name}, room="maison_dashboard")
...
def _alert_low_rating(fb) -> None:
	...
	users = boutique_manager_users(fb.boutique) + frappe.get_all("Has Role", filters={"role": "Maison Head Office", "parenttype": "User"}, pluck="parent")
	...
	_notify(users, subject, "Maison Feedback", fb.name, body)
	try:
		...
		if emails and can_send and not frappe.flags.in_test:
			frappe.sendmail(recipients=emails, subject=subject, message=body, reference_doctype="Maison Feedback", reference_name=fb.name, delayed=True)
	except Exception:
		frappe.log_error(frappe.get_traceback(), "maison feedback alert mail")
	finally:
		frappe.clear_messages()  # never leak desk messages (e.g. "setup outgoing email") to the guest page
	frappe.publish_realtime("maison_feedback_alert", {"boutique": fb.boutique, "rating": fb.rating, "name": fb.name}, room="maison_dashboard")
...
def list(boutique: Optional[str] = None, from_date: Optional[str] = None, to_date: Optional[str] = None, status: Optional[str] = None, limit: int = 50) -> builtins.list[dict[str, Any]]:  # noqa: A001 - API name
	...
	rows = frappe.get_all("Maison Feedback", filters=filters, fields=FIELDS, order_by="submitted_at desc", limit=min(cint(limit) or 50, 500))
	for r in rows:
		r["associate_name"] = frappe.db.get_value("Maison Associate", r.associate, "full_name") if r.associate else None
...
def summary(days: int = 30) -> dict[str, Any]:
	...
	rows = frappe.get_all("Maison Feedback", filters=filters, fields=["boutique", "rating", "comment", "submitted_at", "name", "status"], order_by="submitted_at desc")
...
def respond(name: str, response: str, status: str = "Responded") -> dict[str, Any]:
	...
	assert_roles("Maison Manager", "Maison Regional", "Maison Head Office", "System Manager")
	doc = frappe.get_doc("Maison Feedback", name)
