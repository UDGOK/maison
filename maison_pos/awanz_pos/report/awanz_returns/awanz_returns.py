"""AWANZ Returns — credit notes by reason / boutique / associate (or line detail): count, units, value, refund methods."""
...
def execute(filters=None):
	...
	if group_by == "Detail":
		...
		columns = [
			col("Credit Note", "invoice", "Link", 160, "Sales Invoice"), col("Date", "date", "Date", 95), col("Boutique", "boutique", "Link", 95, "AWANZ Store"),
	...
	for a in sorted(agg.values(), key=lambda x: -x["value"]):
		...
		data.append(a)
	first = {"Boutique": col("Boutique", "key", "Link", 110, "AWANZ Store"), "Associate": col("Associate", "key", "Link", 200, "AWANZ Associate")}.get(group_by, col("Reason", "key", "Data", 140))
