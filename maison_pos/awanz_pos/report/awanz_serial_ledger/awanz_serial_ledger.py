"""AWANZ Serial Ledger — every serial number: received → sold / returned / transferred, and where it is now."""
...
def execute(filters=None):
	# --- v0.8 QA D-14 — the only AWANZ report that skipped `normalize_filters` ---
	...
	wh_to_b = {r.warehouse: r.name for r in frappe.get_all("AWANZ Store", fields=["name", "warehouse"])}
	damaged = {r.damaged_warehouse: r.name for r in frappe.get_all("AWANZ Store", fields=["name", "damaged_warehouse"]) if r.damaged_warehouse}
