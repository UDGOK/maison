...
def pricing_rule_title(boutique: str, item_code: str) -> str:
	return f"AWANZ {boutique} {item_code}"


class AWANZPriceChangeRequest(Document):
	# -- lifecycle ----------------------------------------------------------
	def validate(self) -> None:
		...
		if not self.warehouse and self.boutique:
			self.warehouse = frappe.db.get_value("AWANZ Store", self.boutique, "warehouse")
...
def count_pending(boutique: str | None = None) -> int:
	...
	return frappe.db.count("AWANZ Price Change Request", filters)
