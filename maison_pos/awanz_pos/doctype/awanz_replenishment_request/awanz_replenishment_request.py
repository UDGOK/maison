"""AWANZ Replenishment Request (v0.6 O/P): a store asks the main warehouse for stock.
...
``AWANZ Replenishment Approval`` moves it Pending Approval → Approved / Rejected; on Approved
...
class AWANZReplenishmentRequest(Document):
	def validate(self) -> None:
		...
		if not self.to_warehouse:
			self.to_warehouse = frappe.db.get_value("AWANZ Store", self.boutique, "warehouse")
		if not self.from_warehouse:
			from maison_pos.shipping import get_main_warehouse

			self.from_warehouse = get_main_warehouse(
				exclude=self.to_warehouse,
				company=frappe.db.get_value("AWANZ Store", self.boutique, "company"),
			)
