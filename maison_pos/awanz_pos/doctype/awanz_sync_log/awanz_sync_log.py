"""AWANZ Sync Log: idempotency ledger keyed by the POS client's offline_uuid."""
...
def record(
	...
	*,
	...
	existing = frappe.db.get_value("AWANZ Sync Log", {"offline_uuid": offline_uuid}, ["name", "attempts"], as_dict=True)
	if existing:
		values["attempts"] = (existing.attempts or 0) + 1
		frappe.db.set_value("AWANZ Sync Log", existing.name, values, update_modified=True)
		return existing.name

	doc = frappe.new_doc("AWANZ Sync Log")
