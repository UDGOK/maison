"""AWANZ Stock Alert: one open row per (item, warehouse) below its reorder level."""
...
class AWANZStockAlert(Document):
	def validate(self) -> None:
		if not self.boutique and self.warehouse:
			self.boutique = frappe.db.get_value("AWANZ Store", {"warehouse": self.warehouse}, "name")
