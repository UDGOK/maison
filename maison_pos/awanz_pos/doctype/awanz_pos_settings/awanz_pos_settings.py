"""AWANZ POS Settings (single): global POS switches merged into ``catalog.bootstrap``."""
...
class AWANZPOSSettings(Document):
...
def _raw_settings() -> dict[str, Any]:
	...
	if frappe.db.exists("DocType", "AWANZ POS Settings"):
		...
		stored = frappe.db.get_singles_dict("AWANZ POS Settings", cast=True)
...
def is_recognition_enabled(boutique: Optional[str] = None) -> bool:
	...
	if boutique:
		override = frappe.db.get_value("AWANZ Store", boutique, "face_recognition_enabled")
		if override == "On":
			enabled = 1
		elif override == "Off":
			enabled = 0
...
def get_pos_settings(boutique: Optional[str] = None) -> dict[str, Any]:
	...
	if boutique:
		override = frappe.db.get_value("AWANZ Store", boutique, "show_product_images")
		if cint(override):
			show_images = 1
