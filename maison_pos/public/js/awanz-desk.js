...
// `frappe.boot.awanz_brand`, which `maison_pos.setup.whitelabel.extend_bootinfo` fills from
// `AWANZ POS Settings` — no tenant name is written here.
//
...
frappe.provide("awanz.whitelabel");

awanz.whitelabel.brand = function () {
	return (frappe.boot && frappe.boot.awanz_brand) || {};
};

awanz.whitelabel.about = function () {
	const b = awanz.whitelabel.brand();
	const product = b.product_name || b.brand_name || frappe.boot.app_name || "";
	const brand_name = b.brand_name || product;

	if (awanz.whitelabel.about_dialog) {
		awanz.whitelabel.about_dialog.show();
		return;
	}
	...
	$(dialog.body).html(
		`<div>
			...
			<hr>
			<h5>${__("Version")}</h5>
			<div id='awanz-about-version' class='text-muted'>${__("Loading versions...")}</div>
			<hr>
...
frappe.ui.misc.about = awanz.whitelabel.about;
