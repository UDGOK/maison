...
* ``user`` / ``boutique`` / ``role`` are **permlevel 1**. They decide who somebody is on this
  ...
  permlevel reset during it, and by :meth:`MaisonAssociate._guard_privileged_fields` inside it.
* ``pin`` / ``pin_hash`` / ``pin_set_on`` / ``failed_pin_attempts`` are **permlevel 2** — no
  Maison role can read them — and ``pin_hash`` is a ``Password`` field, so the value physically
...
* the ``Maison *`` Frappe role is synced from ``role``, but never above the granting user's own
...
class MaisonAssociate(Document):
	...
	def _sync_user_role(self) -> None:
		"""Ensure the linked User carries the matching ``Maison *`` role — and only that one.
		...
		to grant a rank above the *granting* user's own, and takes back the Maison role a
	...
	def _upgrade_hash(self, pin: str, stored: Optional[str]) -> None:
		...
		try:
			...
			frappe.log_error(frappe.get_traceback(), "maison pin hash upgrade")
...
def upsert(
	...
	existing = frappe.db.exists("Maison Associate", {"user": user})
	if existing:
		doc = frappe.get_doc("Maison Associate", existing)
		...
		doc = frappe.new_doc("Maison Associate")
		doc.user = user
		doc.boutique = boutique
		doc.role = role
