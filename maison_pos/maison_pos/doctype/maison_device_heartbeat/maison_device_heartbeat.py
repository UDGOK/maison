"""Maison Device Heartbeat: one row per (boutique, device) updated every 60s by the PWA."""
...
class MaisonDeviceHeartbeat(Document):
...
def upsert_heartbeat(
	...
	if name:
		frappe.db.set_value("Maison Device Heartbeat", name, values, update_modified=False)
	else:
		doc = frappe.new_doc("Maison Device Heartbeat")
