...
SCREENS: list[tuple[str, str, str, tuple[str, ...]]] = [
	("/pos", "Point of sale", "Ring sales, returns, receiving and cycle counts on the till.",
		("Maison Associate", "Maison Manager", "Maison Regional", "Maison Head Office", "System Manager")),
	("/maison-dashboard", "Command dashboard", "Live sales across every store, products, clients and insights.",
		("Maison Regional", "Maison Head Office", "System Manager")),
	("/warehouse", "Warehouse desk", "Approve store replenishment, pick, buy labels and ship.",
		("Maison Warehouse Admin", "Maison Head Office", "System Manager")),
...
def get_context(context):
	...
	context.store = ""
	if not context.is_guest and frappe.db.exists("DocType", "Maison Associate"):
		row = frappe.get_all(
			"Maison Associate",
		...
		if row and row[0].boutique:
			context.store = frappe.db.get_value("Maison Boutique", row[0].boutique, "boutique_name") or row[0].boutique
	return context
