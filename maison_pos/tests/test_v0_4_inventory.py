...
class TestInventory(FrappeTestCase):
	...
	def test_scan_creates_alert_once_and_resolves(self):
		...
		names = [n for n in first["created"] if frappe.db.get_value("Maison Stock Alert", n, "item_code") == "AC-010"]
		self.assertEqual(len(names), 1, first)
		alert = frappe.get_doc("Maison Stock Alert", names[0])
		...
		self.assertFalse([n for n in second["created"] if frappe.db.get_value("Maison Stock Alert", n, "item_code") == "AC-010"])
		self.assertEqual(frappe.db.count("Maison Stock Alert", {"item_code": "AC-010", "warehouse": self.warehouse, "status": ("in", ("Open", "Acknowledged"))}), 1)
		# notification reached the boutique manager
		self.assertTrue(frappe.db.exists("Notification Log", {"for_user": NYC_MANAGER, "document_type": "Maison Stock Alert"}))
		...
		self.assertEqual(frappe.db.get_value("Maison Stock Alert", alert.name, "status"), "Resolved")
	...
	def test_associate_cannot_resolve_but_can_request_transfer(self):
		...
		name = next(n for n in created if frappe.db.get_value("Maison Stock Alert", n, "item_code") == "AC-009")
		...
		self.assertEqual(mr.items[0].from_warehouse, frappe.db.get_value("Maison Boutique", "CHI-OAK", "warehouse"))
		self.assertEqual(flt(mr.items[0].qty), 4)
		self.assertEqual(frappe.db.get_value("Maison Stock Alert", name, "material_request"), mr.name)
	...
	def test_cycle_count_reports_unaccounted_serials_and_drafts_reconciliation(self):
		...
		cc = frappe.get_doc("Maison Cycle Count", res["cycle_count"])
	...
	def test_seed_created_damaged_warehouses_readers_and_samples(self):
		...
		self.assertGreaterEqual(frappe.db.count("Maison Stock Alert", {"status": ("in", ("Open", "Acknowledged"))}), 2)
