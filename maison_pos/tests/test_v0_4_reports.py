...
class TestReports(FrappeTestCase):
	...
	def test_tax_summary_matches_invoices(self):
		res = _run("AWANZ Sales Tax Summary", boutique=NYC)
		rows = res["rows"]
		...
		self.assertEqual(row["tax_template"], frappe.db.get_value("AWANZ Store", NYC, "tax_template"))

	def test_daily_sales_nets_returns(self):
		res = _run("AWANZ Daily Sales", boutique=NYC)
	...
	def test_item_associate_returns_reports(self):
		by_item = _run("AWANZ Sales by Item", boutique=NYC, group_by="Item")["rows"]
		...
		by_dept = _run("AWANZ Sales by Item", boutique=NYC, group_by="Department")["rows"]
		self.assertTrue(any(r["key"] == "Accessories" for r in by_dept))
		assoc = _run("AWANZ Sales by Associate", boutique=NYC)["rows"]
		self.assertEqual(sum(r["tickets"] for r in assoc), sum(1 for i in self._invoices_today() if not i.is_return))
		ret = _run("AWANZ Returns", boutique=NYC, group_by="Reason")["rows"]
		...
		detail = _run("AWANZ Returns", boutique=NYC, group_by="Detail")["rows"]
		...
		heat = _run("AWANZ Hourly Sales Heatmap", boutique=NYC)
		self.assertTrue(heat["rows"] and heat["chart"])
		clients = _run("AWANZ Client Purchases", boutique=NYC)["rows"]
		...
		ledger = reports.run("AWANZ Serial Ledger", {"item_code": "TP-001"})["rows"]
	...
	def test_period_comparison_and_scoping(self):
		...
		with self.assertRaises(frappe.PermissionError):
			reports.run("AWANZ Daily Sales", {"boutique": "CHI-OAK", "from_date": nowdate(), "to_date": nowdate()})
		other = reports.run("AWANZ Daily Sales", {"from_date": nowdate(), "to_date": nowdate()})
		self.assertTrue(all(r["boutique"] == NYC for r in other["rows"]))

	def test_csv_export(self):
		reports.export("AWANZ Sales Tax Summary", {"boutique": NYC, "from_date": nowdate(), "to_date": nowdate()})
		...
		reports.export("AWANZ Commission Statement", {"boutique": NYC, "from_date": nowdate(), "to_date": nowdate()})
		self.assertEqual(frappe.response["type"], "download")
