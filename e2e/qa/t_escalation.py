...
print("=== E1: Manager writes to ANOTHER store's Maison Associate (cross-store write) ===")
tgt = "ok.etul.a1@cloudchaserz.example"   # OK-ETUL associate
orig = getv(adm, "Maison Associate", tgt, "full_name")
print("  target OK-ETUL associate full_name (orig):", orig)
r = mgr.post("frappe.client.set_value", doctype="Maison Associate", name=tgt, fieldname="full_name", value="QA-SECUX-MARK")
print("  manager set_value cross-store:", summ(r)[:160])
now = getv(adm, "Maison Associate", tgt, "full_name")
print("  full_name after manager write:", now, " -> CROSS_STORE_WRITE_OK =", now=="QA-SECUX-MARK")
if now == "QA-SECUX-MARK":
    rv = adm.post("frappe.client.set_value", doctype="Maison Associate", name=tgt, fieldname="full_name", value=orig)
    print("  reverted:", getv(adm,"Maison Associate",tgt,"full_name"))

print("=== E2: Manager repoints their OWN boutique to another store (self-escalation) ===")
me = "ok.mingo.manager@cloudchaserz.example"
orig_b = getv(adm, "Maison Associate", me, "boutique")
print("  own boutique (orig):", orig_b)
r = mgr.post("frappe.client.set_value", doctype="Maison Associate", name=me, fieldname="boutique", value="OK-ETUL")
print("  manager repoint self:", summ(r)[:160])
now_b = getv(adm, "Maison Associate", me, "boutique")
print("  own boutique after:", now_b, " -> SELF_REPOINT_OK =", now_b=="OK-ETUL")
if now_b != orig_b:
    adm.post("frappe.client.set_value", doctype="Maison Associate", name=me, fieldname="boutique", value=orig_b)
    print("  reverted own boutique ->", getv(adm,"Maison Associate",me,"boutique"))

print("=== E3: Manager promotes an associate role Associate->Manager (own store) ===")
a1 = "ok.mingo.a1@cloudchaserz.example"
orig_r = getv(adm, "Maison Associate", a1, "role")
r = mgr.post("frappe.client.set_value", doctype="Maison Associate", name=a1, fieldname="role", value="Manager")
now_r = getv(adm, "Maison Associate", a1, "role")
print(f"  role {orig_r}->{now_r}  ROLE_WRITE_OK={now_r=='Manager'}  ", summ(r)[:100])
if now_r != orig_r:
    adm.post("frappe.client.set_value", doctype="Maison Associate", name=a1, fieldname="role", value=orig_r)
    print("  reverted role ->", getv(adm,"Maison Associate",a1,"role"))

print("=== E4: Associate escalates own Frappe role (add Maison Manager to own User) ===")
...
r2 = asc.resource("User/ok.mingo.a1@cloudchaserz.example", method="PUT",
                  json={"roles":[{"role":"Maison Manager"}]})
print("  associate add role via /api/resource PUT:", r2.status_code, r2.text[:120])

print("=== E5: Associate writes to own Maison Associate row (self boutique/role) ===")
r = asc.post("frappe.client.set_value", doctype="Maison Associate", name=a1, fieldname="boutique", value="OK-ETUL")
