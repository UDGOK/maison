...
record('/warehouse desk opens for the AWANZ Warehouse Admin (no permission gate)',
...
const before = await wall.evaluate(() => window.__awanzLastWallPrint || null)
...
const newShip = (await a.list('AWANZ Shipment', { replenishment_request: S.RUI }, ['name', 'status'], 5))[0]
...
const job = await wall.evaluate(() => window.__awanzLastWallPrint || null)
...
const pl = await a.ctx.request.get(job?.url || `/printview?doctype=AWANZ%20Shipment&name=${newShip.name}&format=AWANZ%20Packing%20List&no_letterhead=1`)
...
const st = await a.value('AWANZ Shipment', S.toPick.ship, ['status'])
