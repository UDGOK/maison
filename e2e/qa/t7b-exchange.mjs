...
try {
  ...
  // ================= 7.9 manager PIN threshold =================
  const big = await sell({ codes: ['SVC-003'], qty: { 'Gift Card $100': 24 } })  // 24 x $100 = 2400 + tax
  record('7.9.0 setup: a sale above the $2,500 manager threshold', !!big.inv && big.total > 2500, `${big.inv?.name} total ${big.total} (${big.pill})`)
