...
record('C · the server holds a Paired AWANZ Salon Session for this POS device', st.session?.status === 'Paired', `${st.session?.token?.slice(0, 10)}… boutique=${st.session?.boutique}`)
...
const inter = (await admin.list('AWANZ Client Interaction', { customer: MEMBER }, ['name', 'type', 'note'], 5)).find((i) => /bigger size/.test(i.note || ''))
...
await sleep(3000)
const fb = (await admin.list('AWANZ Feedback', { sales_invoice: inv?.name }, ['name', 'rating', 'comment', 'boutique', 'source'], 5).catch(() => []))
const fb2 = fb.length ? fb : await admin.list('AWANZ Feedback', { boutique: L.STORE }, ['name', 'rating', 'comment', 'sales_invoice'], 5)
...
const prof = await admin.value('AWANZ Client Profile', MEMBER, ['private_viewing_invite', 'private_viewing_invite_on'])
...
if (await toggle.count()) {
  ...
  if (conc) {
    ...
    const p2 = await admin.doc('AWANZ Client Profile', MEMBER)
...
const sess = await admin.value('AWANZ Salon Session', st.session.token, ['status'])
