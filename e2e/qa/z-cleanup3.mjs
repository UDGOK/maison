...
for (const f of await admin.list('AWANZ Feedback', { comment: ['like', '%QA4%'] }, ['name'], 10)) {
  for (const n of await admin.list('Notification Log', { document_name: f.name }, ['name'], 10).catch(() => [])) await del('Notification Log', n.name)
  await del('AWANZ Feedback', f.name)
}
for (const e of await admin.list('AWANZ Web Enquiry', { customer_name: ['like', 'QA4%'] }, ['name'], 10)) await del('AWANZ Web Enquiry', e.name)
record('cleanup · test feedback + enquiry removed (they would skew the HQ tiles)',
  (await admin.list('AWANZ Feedback', { comment: ['like', '%QA4%'] }, ['name'], 5)).length === 0 && (await admin.list('AWANZ Web Enquiry', { customer_name: ['like', 'QA4%'] }, ['name'], 5)).length === 0,
...
out.kept = {
  ...
  coupons: (await admin.list('AWANZ Coupon', {}, ['name', 'enabled'], 20)),
  consents: (await admin.list('AWANZ Biometric Consent', {}, ['name', 'status', 'customer'], 10)),
