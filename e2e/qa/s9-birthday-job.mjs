...
const settings = await admin.doc('AWANZ POS Settings')
...
const profiles = await admin.list('AWANZ Client Profile', {}, ['customer', 'birthday'], 500)
...
const oldBirthday = (await admin.value('AWANZ Client Profile', MEMBER, ['birthday'])).birthday
await admin.post('frappe.client.set_value', { doctype: 'AWANZ Client Profile', name: MEMBER, fieldname: 'birthday', value: `1990-${md}` })
const couponsBefore = await admin.list('AWANZ Coupon', { code: ['like', 'BDAY%'] }, ['name', 'customer'], 100)
...
await sleep(4000)
const couponsAfter = await admin.list('AWANZ Coupon', { code: ['like', 'BDAY%'] }, ['name', 'customer', 'discount_type', 'value', 'usage', 'max_uses', 'valid_from', 'valid_upto', 'enabled', 'title'], 100)
...
if (mine) {
  ...
  const inter = await admin.list('AWANZ Client Interaction', { customer: MEMBER, type: 'Birthday' }, ['name', 'note'], 5)
...
await sleep(3000)
const couponsAfter2 = await admin.list('AWANZ Coupon', { code: ['like', 'BDAY%'] }, ['name'], 100)
