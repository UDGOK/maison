...
const fdoc = rows.length ? await admin.doc('Maison Feedback', rows[0].name) : null
...
const leak = await shopper.raw('frappe.client.get_list', { doctype: 'Maison Feedback', fields: JSON.stringify(['name', 'rating', 'comment']), limit_page_length: 5 })
