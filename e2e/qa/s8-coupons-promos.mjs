...
for (const d of defs) {
  if (!(await admin.list('Maison Coupon', { code: d.code }, ['name'])).length) await admin.post('frappe.client.insert', { doc: { doctype: 'Maison Coupon', enabled: 1, ...d } })
...
const red = await admin.list('Maison Coupon Redemption', { coupon: defs[0].code }, ['name', 'sales_invoice', 'amount', 'customer'], 5)
const cpn = await admin.value('Maison Coupon', defs[0].code, ['used_count'])
...
const cals = await admin.list('Maison Promotion Calendar', {}, ['name', 'title', 'month', 'status', 'coupon', 'campaign', 'sent_on'], 20, 'month asc')
...
const calDoc = thisMonth ? await admin.doc('Maison Promotion Calendar', thisMonth.name) : null
...
const entries = await admin.list('Maison Giveaway Entry', { customer: MEMBER }, ['name', 'giveaway', 'entries', 'sales_invoice'], 10)
...
const gname = (await admin.post('frappe.client.insert', { doc: { doctype: 'Maison Giveaway', title: `QA4 test giveaway ${TAG} (ignore)`, status: 'Open', prize_item: 'ACC-002', prize_description: 'QA4 test prize', start_date: plus(0), end_date: plus(0), entry_rule: 'Per amount', amount_per_entry: 25, requires_member: 1 } })).name
...
await sleep(1500)
const myEntries = await admin.list('Maison Giveaway Entry', { giveaway: gname }, ['name', 'customer', 'entries', 'sales_invoice'], 10)
...
const drawn = await admin.doc('Maison Giveaway', gname)
