...
const SHOTS = path.join(__dirname, process.env.SHOTS_DIR || 'cloud-shots')
...
// Playwright storageState carrying the Administrator session cookie (used instead of POST /api/method/login)
...
async function adminApi() {
  ...
  let ctx
  if (ADMIN_SID) {
    ctx = await request.newContext({ baseURL: BASE, storageState: adminStorageState() })
...
async function receiptState(page) {
  ...
  const reason = /Rejected/i.test(pill) ? (await page.locator('.receipt-view .rejected, .receipt-view [class*=reject], .receipt-view .error').first().innerText().catch(() => '')).replace(/\s+/g, ' ').slice(0, 400) : ''
  return { pill, invoice: (inv || '').trim(), uuid, reason }
}
...
// ================================================================================
// BRIDGE=1: relay all browser traffic through Node (sandbox egress proxy resets Chromium TLS) — see cloud-bridge.mjs
...
const posWsFrames = []
if (BRIDGE) await installBridge(context, { isOffline: () => offlineFlag, onWsFrame: (f) => posWsFrames.push(f) })
...
try {
  ...
  record('CASH sale synced + server invoice verified', rs.pill === 'Synced' && issues1.length === 0, `${rs.pill} ${doc1?.name} total ${doc1?.grand_total} ${issues1.join('; ')}${rs.reason ? ' | server: ' + rs.reason : ''}`)
  ...
  record('CARD sale (simulated reader) synced + verified', rs.pill === 'Synced' && issues2.length === 0, `${rs.pill} ${doc2?.name} ref ${doc2?.maison_terminal_ref} ${issues2.join('; ')}${rs.reason ? ' | server: ' + rs.reason : ''}`)
  await page.click('button:has-text("Done")')
  ...
  record('queue drained after reconnect + invoice server-side', rs.pill === 'Synced' && issues3.length === 0 && !/queued/.test(statusTxt2), `${rs.pill} ${doc3?.name} topbar "${statusTxt2}" ${issues3.join('; ')}${rs.reason ? ' | server: ' + rs.reason : ''}`)
  await page.click('button:has-text("Done")')

  // 5. dashboard live update
  const dctx = await browser.newContext({ viewport: { width: 1920, height: 1080 }, baseURL: BASE, ...(ADMIN_SID ? { storageState: adminStorageState() } : {}) })
  const dl = ADMIN_SID ? await dctx.request.get('/api/method/frappe.auth.get_logged_user') : await dctx.request.post('/api/method/login', { data: ADMIN })
  const dpage = await dctx.newPage()
  wireConsole(dpage, 'dashboard')
  const socketFrames = []
  if (BRIDGE) await installBridge(dctx, { onWsFrame: (f) => socketFrames.push(f) })
  ...
  const envCheck = async (pg) => pg.evaluate(async () => {
    ...
    const unbounded = [...document.fonts].filter((f) => /Unbounded/i.test(f.family))
    const links = [...document.querySelectorAll('link[rel=stylesheet]')].map((l) => l.href).filter((h) => /fonts\.googleapis/.test(h))
    const usesUnbounded = [...document.querySelectorAll('h1,h2,.logo,.brand,.topbar,.kpi,.total-amt,.kpi-val,[class*=title]')].some((el) => /Unbounded/i.test(getComputedStyle(el).fontFamily))
    let sw = { supported: 'serviceWorker' in navigator, regs: [] }
    if (sw.supported) {
      const regs = await navigator.serviceWorker.getRegistrations()
      sw.regs = regs.map((r) => ({ scope: r.scope, state: (r.active || r.waiting || r.installing)?.state, script: (r.active || r.waiting || r.installing)?.scriptURL }))
      sw.controller = navigator.serviceWorker.controller?.scriptURL || null
    }
    return { googleLinks: links, unboundedFaces: unbounded.map((f) => `${f.weight}:${f.status}`), unboundedLoaded: unbounded.some((f) => f.status === 'loaded'), usesUnbounded, facesCount: faces.length, sw }
  })
  const posEnv = await envCheck(page)
  const dashEnv = await envCheck(dpage)
  record('Google Fonts: Unbounded loaded (POS)', posEnv.unboundedLoaded, `faces ${JSON.stringify(posEnv.unboundedFaces)} usedInComputedStyle=${posEnv.usesUnbounded} links=${JSON.stringify(posEnv.googleLinks)}`)
  record('Google Fonts: Unbounded loaded (dashboard)', dashEnv.unboundedLoaded, `faces ${JSON.stringify(dashEnv.unboundedFaces)} usedInComputedStyle=${dashEnv.usesUnbounded}`)
  record('service worker registered (POS)', posEnv.sw.regs.length > 0, JSON.stringify(posEnv.sw))
  log('  dashboard SW:', JSON.stringify(dashEnv.sw))
  ...
  if (!inv4) log('  sale 4 not synced:', rs.pill, rs.reason)
  const t0 = Date.now()
  let seen = false
...
fs.writeFileSync(path.join(__dirname, process.env.RESULTS || 'results.cloud.json'), JSON.stringify(out, null, 2))
