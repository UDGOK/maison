...
//       CSRF scraped from /pos), ASSOC_USER/ASSOC_PWD, SHOTS_DIR (default cloud-shots-v02), RESULTS (default results.v02.cloud.json),
//       BRIDGE=1 (relay browser traffic through Node, see cloud-bridge.mjs)
...
const SHOTS = path.join(__dirname, process.env.SHOTS_DIR || 'cloud-shots-v02')
...
const MANAGER = { usr: process.env.MANAGER_USER || 'chi.oak.manager@maison.example', pwd: process.env.MANAGER_PWD || 'maison123' }
...
// ---- admin API helper (Administrator sid cookie, GET only) + manager context (login + CSRF, for the upload) ----
...
async function managerApi() {
  ...
  const r = await ctx.post('/api/method/login', { data: MANAGER })
  if (!r.ok()) throw new Error(`manager login failed ${r.status()} ${(await r.text()).slice(0, 200)}`)
  const who = await (await ctx.get('/api/method/frappe.auth.get_logged_user')).json()
...
let manager = null
try {
  manager = await managerApi()
  record('manager login (POST /api/method/login) + CSRF token scraped from /pos', manager.user === MANAGER.usr && !!manager.csrf, `user=${manager.user} csrf=${manager.csrf ? manager.csrf.slice(0, 8) + '…' : 'NONE'}`)
} catch (e) {
  record('manager login (POST /api/method/login) + CSRF token scraped from /pos', false, String(e))
}

// 0. Manager uploads a tile photo via the API (small generated PNG), and seed data we scan.
let imageUrl = null
try {
  if (!manager) throw new Error('no manager session')
  const up = await manager.upload('maison_pos.api.catalog.upload_item_image', { item_code: IMAGE_ITEM }, { name: 'pocket-square.png', mimeType: 'image/png', buffer: makePng() })
  ...
  const img = await manager.ctx.get(imageUrl)
...
if (BRIDGE) await installBridge(context)
...
try {
  ...
  // 0b. gold palette live: computed body background ~ #0B0B0A
  const bodyBg = await page.evaluate(() => getComputedStyle(document.body).backgroundColor)
  const ground = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue('--ground').trim())
  const ch = (bodyBg.match(/\d+/g) || []).map(Number)
  const bgOk = ch.length >= 3 && Math.abs(ch[0] - 11) <= 2 && Math.abs(ch[1] - 11) <= 2 && Math.abs(ch[2] - 10) <= 2
  record('gold palette live: computed body background ~ #0B0B0A', bgOk, `body=${bodyBg} --ground=${ground}`)
...
if (receiptToken) {
  ...
  // render the guest receipt page in a fresh (cookie-less) browser context — desktop and iPhone
  for (const [tag, vp] of [['desktop', { width: 1366, height: 1024 }], ['phone', { width: 390, height: 844, deviceScaleFactor: 2, isMobile: true, hasTouch: true }]]) {
    const gctx = await browser.newContext({ viewport: { width: vp.width, height: vp.height }, deviceScaleFactor: vp.deviceScaleFactor, isMobile: vp.isMobile, hasTouch: vp.hasTouch, baseURL: BASE })
    if (BRIDGE) await installBridge(gctx)
    const gp = await gctx.newPage()
    wireConsole(gp, 'guest-' + tag)
    try {
      const resp = await gp.goto(`/r/${receiptToken}`, { waitUntil: 'networkidle' })
      // guest identity: the server stamps sid=Guest / user_id=Guest on the response (get_logged_user is 403 for guests)
      const ck = Object.fromEntries((await gctx.cookies(BASE)).map((c) => [c.name, c.value]))
      const who = { message: ck.user_id === 'Guest' && ck.sid === 'Guest' ? 'Guest' : `${ck.user_id}/${ck.sid?.slice(0, 6)}` }
      ...
      record(`GET /r/<token> renders as guest (${tag}) with QR image`, resp.status() === 200 && who.message === 'Guest' && qrN === 1 && !!qrInfo?.complete && qrInfo.w > 0 && hasBoutique,
        `${resp.status()} user=${who.message} qr=${JSON.stringify(qrInfo)} boutique=${hasBoutique} body=${bg}`)
    } catch (e) {
      record(`GET /r/<token> renders as guest (${tag}) with QR image`, false, String(e).slice(0, 200))
    }
    await gctx.close()
  }
} else {
...
if (BRIDGE) await installBridge(phone)
...
fs.writeFileSync(path.join(__dirname, process.env.RESULTS || 'results.v02.cloud.json'), JSON.stringify({ results, console: consoleLog }, null, 2))
