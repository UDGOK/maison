...
//
// Flow: enable recognition for CHI-OAK (Administrator) → associate: Looking → New client → enrol with
// phone (hold-to-agree) → server Customer + Active consent + 3 templates + Enrolled event → reload →
// Recognised (distance < threshold, score = 1 − d/1.2) → Undo event → different face → no match →
// decline creates the customer without biometrics → manager revoke → purge verified server-side →
// offline enrolment queues and replays on reconnect → dashboard live_summary.recognition counters →
// recognition switched off again.
...
const SHOTS = path.join(__dirname, 'shots-v03')
...
async function adminApi() {
  ...
  const api = {
    ...
    // child tables are not listable through frappe.client.get_list; recognition.status counts them server-side
...
async function posContext(browser, user, tag) {
...
/** Fresh device: wipe localStorage / IndexedDB and turn the e2e hook on (needed for state introspection). */
...
/** Deterministic synthetic embedding for the hook fallback (non-unit, like a real descriptor). */
const FAKE_EMBEDDING = Array.from({ length: 128 }, (_, i) => Math.sin(i * 0.37 + 1) * 0.19)

/** Wait for the detector's verdict; inject an embedding through the hook if it never sees a face. */
...
/** If the capture step stalls (no face), feed samples through the hook. Returns true when real captures were used. */
...
async function waitEnrolClosed(page, ms = 20000) {
  const t0 = Date.now()
...
try {
  ...
  record('recognition enabled for CHI-OAK via API (Administrator)', s.face_recognition_enabled === 1 && s.match_threshold === 0.6 && s.match_distance_threshold === 0.6 && s.recognition_model === MODEL,
    `face_recognition_enabled=${s.face_recognition_enabled} match_threshold=${s.match_threshold} (distance) model=${s.recognition_model} consent v${s.consent_text_version}`)
  // earlier runs enrolled the same synthetic face: purge so the first verdict is "New client"
  const stale = await admin.list('Customer', { maison_face_consent: 1 }, ['name'], 100)
  for (const c of stale) await admin.post('maison_pos.api.recognition.revoke', { customer: c.name, reason: 'e2e reset' })
  log(`  reset: revoked ${stale.length} previously consented client(s)`)
  ...
  record('recognition enabled for CHI-OAK via API (Administrator)', false, String(e))
}
...
let customerA = null
{
  ...
  const { context, page } = await posContext(browser, ASSOC, 'faceA')
  try {
    await freshDevice(page)
    await unlock(page, ASSOC)
    const looking = await waitTile(page, ['looking', 'new', 'recognised'], 40000)
    record('tile reaches Looking (camera + models served by the bench)', !!looking, `tile=${looking?.tile} cached=${looking?.cached}`)
    ...
    record('first verdict is New client (no consented templates)', v1.state?.tile === 'new', `${v1.real ? 'real on-device detection' : 'via test hook'} · chip "${chip}" · last=${JSON.stringify(v1.state?.last)}`)
    ...
    record('enrolment completed and client attached to the sale', closed && attached === NAME_A, `${realCapture ? '3 real captures' : 'samples via hook'} · attached "${attached}"`)
    ...
    if (customerA) {
      ...
      record('server: 3 face templates (128-d raw descriptors, no images)', st.templates === 3 && tpls.length === 3 && tpls.every((t) => t.dims === 128 && t.model === MODEL && t.embedding.length === 128) && norms.every((n) => n > 1.05) && !JSON.stringify(tpls).includes('data:image'),
        `status.templates=${st.templates} rows=${tpls.length} ‖d‖=${norms.map((n) => n.toFixed(2)).join('/')} (raw, not unit-normalised)`)
    ...
    record('same face after reload → Recognised (euclidean distance < threshold)', v2.state?.tile === 'recognised' && rec?.customer === customerA?.name && typeof rec?.distance === 'number' && rec.distance < 0.6 && rec.distance < (th ?? 0.6),
    ...
    if (customerA) {
      ...
      record('server: Matched + Undone events logged', matched.length >= 1 && undone.length >= 1, `matched=${matched.length} (score ${matched[0]?.score}) undone=${undone.length} (score ${undone[0]?.score})`)
    }
  } catch (e) {
...
// ---------------------------------------------------------------------------------
// 2. Face B (different person) against A's templates → no match → decline creates the customer without biometrics
let customerB = null
{
  ...
  try {
    ...
    record("different face (B) against A's templates → New client (no false match)", v.state?.tile === 'new' && !v.state?.recognised,
    ...
    if (v.real && customerA) {
      const nm = await admin.events(customerA.name, 'NoMatch').catch(() => [])
      log(`  NoMatch events referencing A: ${nm.length} (NoMatch events carry no customer by design)`)
    }
...
// 3. Manager revoke from the Client screen → purge verified server-side
{
  ...
  try {
    ...
    if (customerA) {
      ...
      record('server: recognition.templates no longer lists the client (raw embeddings, threshold_distance 0.6)', !tl.templates.some((t) => t.customer === customerA.name) && tl.threshold_distance === 0.6, `rows=${tl.templates.length}`)
    }
...
// ---------------------------------------------------------------------------------
// 4. Offline enrolment queues (provisional client) and replays on reconnect
let customerOff = null
{
  const browser = await launch(FACE_A)
  const { context, page } = await posContext(browser, ASSOC, 'offline')
  try {
    await freshDevice(page)
    await unlock(page, ASSOC)
    await waitTile(page, ['looking', 'new', 'recognised'], 40000)
...
// ---------------------------------------------------------------------------------
// 5. Dashboard counters + switch recognition off again
try {
...
try {
  ...
  record('recognition switched off again for the dev site', false, String(e))
}
await admin.dispose()

// ---------------------------------------------------------------------------------
const failed = results.filter((r) => !r.ok)
fs.writeFileSync(path.join(__dirname, 'results.v03.json'), JSON.stringify({ base: BASE, run: RUN, results, console: consoleLog }, null, 2))
